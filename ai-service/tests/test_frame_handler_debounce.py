...
def test_low_confidence_reading_is_not_flagged_as_anomaly():
    """Below the confidence floor, a non-Screen reading should be treated as
    inconclusive rather than a gaze_away candidate at all."""
    low_confidence_gaze = {'direction': 'Right', 'confidence': 0.1, 'model_available': True}
    is_away = (
        low_confidence_gaze.get('direction') != 'Screen'
        and low_confidence_gaze.get('confidence', 0.0) >= fh._GAZE_CONFIDENCE_THRESHOLD
    )
