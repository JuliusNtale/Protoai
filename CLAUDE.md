...
3. Logs behavioural anomalies (gaze away, head turned, tab switch, multiple faces, identity mismatch) to a per-session warning count for lecturer/admin review — the exam is never auto-submitted or locked because of them (removed 2026-07-03); the student always completes the exam via manual submit or the time-up path
...
- **Warnings are logged, not enforced (auto-submit/lock removed 2026-07-03)**: sustained anomaly ≥2s across all anomaly types (gaze away, head turned, face absent, multiple faces — `GAZE_AWAY_SECONDS`/`HEAD_TURNED_SECONDS`/`FACE_ABSENT_SECONDS`/`MULTIPLE_FACES_SECONDS`, all default 2) is gated by confidence + persistence debouncing so the faster window doesn't produce false positives — each confirmed anomaly increments `ExamSession.warning_count` and writes a `BehavioralLog` row via `POST /api/sessions/log`, purely for lecturer/admin post-exam review (`GET /api/sessions`, reports). Nothing in that path locks the session, auto-submits it, or shows a warning count/modal to the student — the student always completes the exam via manual submit or the time-up path. This reverses the earlier "3-strike auto-submit + email" design (`session_locked` socket event and the `force_lock`/`warning_count >= 3` branch in `sessions/routes.py::log_event` were removed); `WARNING_COOLDOWN_SECONDS` (default 5s) still controls how often the SAME continuously-sustained anomaly re-logs, it just no longer escalates to anything.
- **Manual lecturer intervention (added 2026-07-03)**: since proctoring anomalies no longer auto-submit or lock a session, lecturers/admins get two human-in-the-loop actions instead, both gated by `roles_required("lecturer", "admin")` + `_can_view_session` (owning lecturer or any admin) in `backend/app/sessions/routes.py`: `POST /sessions/<id>/warn` sends a real-time message to the student (increments `warning_count`/logs a `lecturer_warning` `BehavioralLog` row exactly like an AI-detected anomaly, but never ends the session — rejected with 409 if the session isn't `active`/`pending`) and `POST /sessions/<id>/terminate` ends it immediately (sets `session_status="terminated"`, `termination_reason`, `submitted_at`; blocks `/submit` afterward). Both push live socket events (`manual_warning` / `session_terminated`) to the student's browser via `_notify_ai_service()` → `ai-service/app.py::/internal/broadcast` → the Socket.IO room named `str(session_id)`, which the student's `webcam_frame` handler joins on every frame and a lecturer's browser joins explicitly via the `join_session_room` socket event (authorized server-side through `GET /sessions/<id>/authorize-viewer`, never trusted client-side). The lecturer UI for both lives in `frontend/app/lecturer/page.tsx` (Live Suspicious Activity feed + Sessions table + report detail view all expose Warn/Terminate); the student reacts to them in `frontend/app/exam/page.tsx` (`manual_warning` → dismissible informational modal, exam keeps running; `session_terminated` → stops monitoring and blocks further interaction).
- **Terminated sessions score whatever was answered so far (added 2026-07-03)**: `POST /sessions/<id>/answer` autosaves one answer at a time into `SessionAnswer` (upserting on its `(session_id, question_id)` unique constraint) as `frontend/app/exam/page.tsx::handleAnswer()` fires — before this endpoint existed, in-progress answers only lived in the browser's React state until the final `/submit` payload, so `terminate_session` had nothing in the database to score and a terminated student got a blanket zero. `terminate_session` now sums marks for every autosaved `SessionAnswer` row with `is_correct=True` into `session.score` before flipping `session_status` to `terminated`, and returns/broadcasts that `score` so both the student's termination modal and the lecturer's session list/report update immediately without a refresh. This is separate from `submit_session`, which still fully overwrites `SessionAnswer` from whatever the client sends at final submit time (freshest client state wins for a normal completion) and is unaffected by the autosave rows.
...
- **Mid-exam identity re-verification (added 2026-07-03)**: the one-time `/verify-identity` check at `/verify` and the `/status`-based entry guard on `/exam` only cover the moment of entry — neither catches someone swapping in for the original student mid-exam without ever leaving/reloading the page. `frame_handler.py::_check_identity_mismatch()` periodically (`IDENTITY_RECHECK_SECONDS`, default 8s) re-runs the FaceNet comparison against the session's registered baseline, throttled since it's heavier than gaze/pose/count. It requires `IDENTITY_MISMATCH_CONFIRM_COUNT` (default 2) CONSECUTIVE mismatched checks before confirming — a lone bad-angle/lighting frame must not penalize a legitimate student, since the consequence (`identity_verified` flips to `False`, blocking any *reload* back into the exam via the `/status` guard) is severe enough to require real confidence. As of 2026-07-03 a confirmed mismatch no longer locks or auto-submits the in-progress session — it's logged like any other anomaly and the student keeps working. The student_id used for comparison is always resolved server-side via a new trusted internal endpoint (`GET /api/sessions/internal/<id>`), never taken from the client — a client-supplied user_id would let an impostor simply claim their own account and match their own face, defeating the check entirely. Shared FaceNet/baseline logic between the one-time and periodic checks lives in `ai-service/services/identity_verifier.py`.
- **Exam clock no longer depends on camera/AI calibration (fixed 2026-08-03)**: `frontend/app/exam/page.tsx`'s countdown only starts once `monitoringCalibrated` is true, and a 20s failsafe effect existed specifically so a slow/failed calibration wouldn't block it forever — but that failsafe itself was gated on `examCameraReady`, so a student who denied camera permission (or had no camera) got a `monitoringCalibrated` that could never become true by any path, freezing the exam clock at its full duration indefinitely. Nothing else in the page depends on camera state — answering, autosave, and manual submit all worked regardless — so the practical effect was an untimed, unmonitored exam with no visible error, not a blocked one. The failsafe now keys off `sessionId` instead of `examCameraReady`, so the clock always starts within ~20s of session confirmation no matter what happens with the camera; AI proctoring itself still requires a working camera/socket connection and is simply skipped (not enforced) when one isn't available, same as before.
- **`alembic.ini` lives in `backend/migrations/`, not `backend/` (fixed 2026-08-04)**: it used to sit at `backend/alembic.ini` (project root), which is what a bare `alembic init migrations` layout produces. `flask db upgrade`/`flask db migrate` (Flask-Migrate) don't read that location — `Migrate.get_config()` always builds the path as `os.path.join(directory, 'alembic.ini')` with `directory` defaulting to `"migrations"`, so those commands require `migrations/alembic.ini` specifically, regardless of what's in the ini file's own `script_location`. Running `flask db upgrade` against the root-level file failed with `FileNotFoundError: migrations/alembic.ini doesn't exist`. The file is now at `backend/migrations/alembic.ini` with its `script_location = migrations` value unchanged — Alembic resolves that path relative to the process's current working directory (`/app` in containers, `backend/` locally), not relative to the ini file itself, so the same value is still correct from the new location. `.github/workflows/cd.yml`'s CD step, which invokes the raw `alembic` CLI (not `flask db`) and previously relied on `alembic.ini` being auto-discovered in its CWD, now passes it explicitly: `alembic -c migrations/alembic.ini upgrade head`. Both call paths were verified against a real Postgres DB after the move.
