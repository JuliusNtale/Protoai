...
- **Warning threshold = 3**: sustained anomaly ≥2s across all anomaly types (gaze away, head turned, face absent, multiple faces — `GAZE_AWAY_SECONDS`/`HEAD_TURNED_SECONDS`/`FACE_ABSENT_SECONDS`/`MULTIPLE_FACES_SECONDS`, all default 2 as of 2026-07-03), gated by confidence + persistence debouncing so the faster window doesn't produce false positives — each confirmed anomaly fires a warning after a persistence + cooldown check (see below). At 3, auto-submit + email. **`WARNING_COOLDOWN_SECONDS` tightened 15s → 5s (2026-07-03)**: this is the gap between re-confirming the SAME continuously-sustained anomaly (the 2s persistence window only gates the first confirmation) — at 15s a student who never broke a violation only re-triggered every 15s, reaching the 3rd strike ~32s in; at 5s the same continuous violation re-fires at ~2s/7s/12s, auto-submitting by ~12s, matching Duolingo-style aggressive escalation.
