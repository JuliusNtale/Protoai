    ...
    return (
      <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#eef2f7] p-6 text-slate-900">
        ...
        <div className="relative w-full max-w-md rounded-3xl border border-slate-200 bg-white/95 p-8 text-center shadow-[0_24px_70px_rgba(15,23,42,0.12)] backdrop-blur">
          ...
          <h1 className="text-xl font-semibold tracking-tight text-slate-900">Loading Admin Dashboard</h1>
        </div>
      </main>
    )
  }
    ...
    <DashboardShell
      ...
      rightTopSlot={
        <div className="flex items-center gap-2">
          <button onClick={() => void load(token)} className="rounded-md border px-3 py-1.5 text-sm font-semibold">
            Refresh
          </button>
    ...
    >
      ...
      <DashboardPanel title="Quick Navigation" subtitle="Open the operational pages from here or from the left sidebar.">
        <div className="grid gap-3 md:grid-cols-3">
          <Link href="/admin/users" className="rounded-xl border border-slate-200 bg-white p-4 transition hover:bg-slate-50">
            <Users className="h-5 w-5 text-[#1a2d5a]" />
            <p className="mt-2 text-sm font-semibold text-slate-900">Users</p>
            <p className="mt-1 text-xs text-slate-600">Provision, manage, and activate/deactivate accounts.</p>
          </Link>
          <Link href="/admin/credentials" className="rounded-xl border border-slate-200 bg-white p-4 transition hover:bg-slate-50">
            <BarChart3 className="h-5 w-5 text-[#1a2d5a]" />
            <p className="mt-2 text-sm font-semibold text-slate-900">Credentials</p>
            <p className="mt-1 text-xs text-slate-600">View generated temporary credentials and export CSV.</p>
          </Link>
          <Link href="/admin/system-logs" className="rounded-xl border border-slate-200 bg-white p-4 transition hover:bg-slate-50">
            <BarChart3 className="h-5 w-5 text-[#1a2d5a]" />
            <p className="mt-2 text-sm font-semibold text-slate-900">Logs</p>
            <p className="mt-1 text-xs text-slate-600">Audit activity, filter events, and monitor session risk.</p>
          </Link>
        </div>
      </DashboardPanel>
    </DashboardShell>
